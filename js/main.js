...
const validators = {
  ...
  dob:      (v) => {
    ...
    const minAge = (typeof window.MIN_REGISTRATION_AGE !== 'undefined') ? parseInt(window.MIN_REGISTRATION_AGE) : 0;
...
function validateInput(input) {
  ...
  for (const rule of rules) {
    ...
    if (!validators[name]?.(testVal, param ? parseInt(param) : undefined)) {
      const minAgeVal = (typeof window.MIN_REGISTRATION_AGE !== 'undefined') ? parseInt(window.MIN_REGISTRATION_AGE) : 0;
      const msgs = {
        ...
        dob:      minAgeVal > 0 ? `You must be at least ${minAgeVal} years old to register.` : 'Enter a valid date of birth.',
      };
...
function initRegisterForm() {
  ...
  uppercaseFields.forEach(id => {
    ...
    // Only INPUT/TEXTAREA support selectionStart/End/setSelectionRange.
    // A <select> (e.g. club_name/association as a dropdown) does NOT, and
    // calling setSelectionRange on one throws "inp.setSelectionRange is not a function",
    // which aborts this forEach and can leave later listeners/handlers unattached.
    if (inp && (inp.tagName === 'INPUT' || inp.tagName === 'TEXTAREA')) {
      ...
      // Selects store discrete values (not free text), so there's no cursor
      // position to preserve. Uppercase the value on change if it's ever
      // populated with free-text-like option values.
      inp.addEventListener('change', () => {
        if (typeof inp.value === 'string') inp.value = inp.value.toUpperCase();
      });
    }
  });
