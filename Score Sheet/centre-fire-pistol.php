<?php
// Score Sheet/centre-fire-pistol.php
...
$eventIdVal = $getEventId($alloc, $EVENTS_MAPPING, 'IS-15');
...
// Fetch document template customization for score_sheet_centre_fire_pistol
$tplStmt = $pdo->prepare("SELECT canvas_data, background_path FROM document_templates WHERE document_type = 'score_sheet_centre_fire_pistol' LIMIT 1");
$tplStmt->execute();
...
<head>
  ...
  <title>Centre Fire Pistol (25M) — Saragarhi Shooting Academy</title>
  <!-- Bootstrap 5 CSS -->
...
<div class="sheet-page">
  ...
  <section class="score-card">
    <div class="score-card-header">
      ...
      <span class="badge">Precision + Rapid Fire Stage</span>
    </div>
    ...
    <div class="table-wrap">
          ...
          <tr>
            ...
            ?>
              <td colspan="<?= $cColspan ?>" rowspan="<?= $cRowspan ?>" class="<?= ($gridColIdx === 0) ? 'row-label' : '' ?>">
                ...
                } elseif (strpos($cText, '{precision_total}') !== false) {
                    echo '<input class="summary-input" type="text" readonly id="precision-total-val" name="precision_total_val" value="' . htmlspecialchars((string)($seriesData['precision_total'] ?? '0')) . '">';
                } elseif (strpos($cText, '{duelling_total}') !== false) {
                    echo '<input class="summary-input" type="text" readonly id="duelling-total-val" name="duelling_total_val" value="' . htmlspecialchars((string)($seriesData['duelling_total'] ?? '0')) . '">';
      ...
      <?php else: ?>
        <thead>
          <tr>
            <th rowspan="2" style="width:110px;">Stage</th>
            <th rowspan="2" style="width:60px;">Seq</th>
            <th colspan="10">Shots</th>
            <th rowspan="2" style="width:90px;">Sub Total</th>
          </tr>
          <tr>
        ...
        <tbody>
          <!-- PRECISION SECTION (6 rows: series 1 to 6) -->
          <?php for ($sIdx = 1; $sIdx <= 6; $sIdx++): ?>
          <tr>
            <?php if ($sIdx === 1): ?>
              <td rowspan="6" class="row-label" style="font-weight:700; text-transform:uppercase; text-align:center; vertical-align:middle; background:rgba(255,255,255,0.03);">Precision</td>
            <?php endif; ?>
            <td class="row-label" style="text-align:center; font-weight:600;"><?= $sIdx ?></td>
            <?php for ($shotIdx = 1; $shotIdx <= 10; $shotIdx++): 
                $val = htmlspecialchars((string)($seriesData[$sIdx][$shotIdx] ?? '0'));
            ?>
            <td><input class="score-input" type="text" maxlength="4" placeholder="0" data-series="<?= $sIdx ?>" data-shot="<?= $shotIdx ?>" value="<?= $val ?>"></td>
            <?php endfor; ?>
          ...
          <tr style="background: rgba(255,255,255,0.05); font-weight:700;">
            <td class="row-label" colspan="12" style="text-align:right; padding-right:1rem;">Precision Total</td>
            <td><input class="summary-input" type="text" readonly id="precision-total-val" value="<?= htmlspecialchars((string)($seriesData['precision_total'] ?? '')) ?>"></td>
          </tr>

          <!-- DUELLING SECTION (6 rows: series 7 to 12) -->
          <?php for ($sIdx = 7; $sIdx <= 12; $sIdx++): 
              $seqNo = $sIdx - 6;
          ?>
          <tr>
            <?php if ($seqNo === 1): ?>
              <td rowspan="6" class="row-label" style="font-weight:700; text-transform:uppercase; text-align:center; vertical-align:middle; background:rgba(255,255,255,0.03);">Duelling</td>
            <?php endif; ?>
            <td class="row-label" style="text-align:center; font-weight:600;"><?= $seqNo ?></td>
            <?php for ($shotIdx = 1; $shotIdx <= 10; $shotIdx++): 
                $val = htmlspecialchars((string)($seriesData[$sIdx][$shotIdx] ?? '0'));
            ?>
            <td><input class="score-input" type="text" maxlength="4" placeholder="0" data-series="<?= $sIdx ?>" data-shot="<?= $shotIdx ?>" value="<?= $val ?>"></td>
          ...
          <?php endfor; ?>
          <tr style="background: rgba(255,255,255,0.05); font-weight:700;">
            <td class="row-label" colspan="12" style="text-align:right; padding-right:1rem;">Duelling Total</td>
            <td><input class="summary-input" type="text" readonly id="duelling-total-val" value="<?= htmlspecialchars((string)($seriesData['duelling_total'] ?? '')) ?>"></td>
          </tr>

          <!-- TOTAL -->
          <tr class="total-row">
            <td class="row-label" colspan="12" style="text-align:right;padding-right:1rem;">Total</td>
          ...
          <tr class="penalty-row" style="background: rgba(231,76,60,0.15) !important;">
            <td class="row-label" colspan="12" style="text-align:right; padding-right:1rem; color:#e74c3c !important; font-weight:700;">Penalty</td>
          ...
          <!-- GRAND TOTAL -->
          <tr class="grand-row">
            <td class="row-label" colspan="12" style="text-align:right;padding-right:1rem;">Grand Total</td>
        ...
        <tbody>
          <tr class="remarks-row">
            <td class="row-label" colspan="12" style="text-align:right;padding-right:1rem;">Remarks</td>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateTotals() {
        ...
        for (let s = 1; s <= 6; s++) {
            ...
            if (seriesTotalInput) {
                ...
                if (hasAnyVal) {
                    precisionTotal += seriesSum;
                    precisionHasVal = true;
                }
            }
        }
        precisionTotal = Math.round(precisionTotal * 100) / 100;
        const precTotalInput = document.getElementById('precision-total-val');
        if (precTotalInput) {
            precTotalInput.value = precisionHasVal ? (precisionTotal % 1 === 0 ? precisionTotal.toString() : precisionTotal.toFixed(1)) : '0';
        }

        let duellingTotal = 0;
        let duellingHasVal = false;
        for (let s = 7; s <= 12; s++) {
            let seriesSum = 0;
            let hasAnyVal = false;
            ...
            if (seriesTotalInput) {
                seriesTotalInput.value = hasAnyVal ? (seriesSum % 1 === 0 ? seriesSum.toString() : seriesSum.toFixed(1)) : '0';
                if (hasAnyVal) {
                    duellingTotal += seriesSum;
                    duellingHasVal = true;
                }
            }
        }
        duellingTotal = Math.round(duellingTotal * 100) / 100;
        const duelTotalInput = document.getElementById('duelling-total-val');
        if (duelTotalInput) {
            duelTotalInput.value = duellingHasVal ? (duellingTotal % 1 === 0 ? duellingTotal.toString() : duellingTotal.toFixed(1)) : '0';
        }

        let grandTotal = (precisionHasVal ? precisionTotal : 0) + (duellingHasVal ? duellingTotal : 0);
        
        // Ultimate fallback: if precision/duelling grouping yielded 0 but score-inputs have values, sum all score-inputs
        if (grandTotal === 0) {
            let allInputsSum = 0;
        ...
        if (totalValInput) {
            totalValInput.value = (precisionHasVal || duellingHasVal) ? ((grandTotal % 1 === 0) ? grandTotal.toString() : grandTotal.toFixed(1)) : '0';
        }
        ...
        if (grandTotalInput) {
            grandTotalInput.value = (precisionHasVal || duellingHasVal) ? ((finalGrand % 1 === 0) ? finalGrand.toString() : finalGrand.toFixed(1)) : '0';
        }
    }
    ...
    window.saveScores = async function() {
        ...
        const pt = document.getElementById('precision-total-val');
        seriesData['precision_total'] = pt ? pt.value : '';
        const dt = document.getElementById('duelling-total-val');
        seriesData['duelling_total'] = dt ? dt.value : '';
